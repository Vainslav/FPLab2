...
-spec delete(any(), rbbag()) -> {err | ok, rbbag()}.
delete(Key, Tree) ->
    NewTree = del(Key, Tree),
    make_black(NewTree).

-spec del(any(), rbbag()) -> {rbbag(), boolean()}.
del(_Key, nil) ->
    nil;
del(Key, #rbbag{data={C, K}, left=L, right=R} = Node) ->
        ...
        e ->
            case {L, R} of
                ...
                _ ->
                    ...
                    NewR = lists:foldl(fun(_, Acc) -> 
                        del(MinKey, Acc)
                    end, R, lists:seq(1, MinCount)),
...
ins(Key, nil) ->
    #rbbag{color=red, data={1, Key}, left=nil, right=nil};
ins(Key, #rbbag{color=_Color, data={C, K}, left=L, right=R} = Node) ->
    case compare(Key, K) of
        e ->
            Node#rbbag{data={C + 1, K}};
        l ->
...
balance(#rbbag{color = black, data = Z, left = #rbbag{color = red, data = Y, left = #rbbag{color = red, data = X, left = A, right = B}, right = C}, right = D}) ->
    #rbbag{color = red, data = Y, left = #rbbag{color = black, data = X, left = A, right = B}, right = #rbbag{color = black, data = Z, left = C, right = D}};
balance(#rbbag{color = black, data = Z, left = #rbbag{color = red, data = X, left = A, right = #rbbag{color = red, data = Y, left = B, right = C}}, right = D}) ->
    #rbbag{color = red, data = Y, left = #rbbag{color = black, data = X, left = A, right = B}, right = #rbbag{color = black, data = Z, left = C, right = D}};
balance(#rbbag{color = black, data = X, left = A, right = #rbbag{color = red, data = Y, left = B, right = #rbbag{color = red, data = Z, left = C, right = D}}}) ->
    #rbbag{color = red, data = Y, left = #rbbag{color = black, data = X, left = A, right = B}, right = #rbbag{color = black, data = Z, left = C, right = D}};
balance(#rbbag{color = black, data = X, left = A, right = #rbbag{color = red, data = Z, left = #rbbag{color = red, data = Y, left = B, right = C}, right = D}}) ->
    #rbbag{color = red, data = Y, left = #rbbag{color = black, data = X, left = A, right = B}, right = #rbbag{color = black, data = Z, left = C, right = D}};
balance(A) ->
    A.


-spec compare(any(), any()) -> term().
compare(A, B) ->
    if
        A > B -> g;
        A < B -> l;
...
foldl(_Fun, Acc, nil) ->
    Acc;
foldl(Fun, Acc, #rbbag{data={Count, Key}, left=L, right=R}) ->
...
foldr(_Fun, Acc, nil) ->
    Acc;
foldr(Fun, Acc, #rbbag{data={Count, Key}, left=L, right=R}) ->
